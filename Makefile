...
SCORER_DIR?=CLEF-HIPE-2020-scorer

SYS_DIR?=systems
SUB_DIR?=$(SYS_DIR)/submitted
SUB_TIMENORM_DIR?=$(SYS_DIR)/time-normalized
SUB_HISTONORM_DIR?=$(SYS_DIR)/histo-normalized
...
DE-GOLD-FILE?=$(RELEASE_DIR)/$(VERSION)/de/HIPE-data-$(VERSION)-test-de.tsv
...
# Train baseline systems and predict bundle 4
baseline-nerc: $(SUB_DIR)/baseline_bundle4_de_1.tsv $(SUB_DIR)/baseline_bundle4_fr_1.tsv $(SUB_DIR)/baseline_bundle4_en_1.tsv

$(SUB_DIR)/baseline_bundle4_%_1.tsv: $(DE-GOLD-FILE) $(FR-GOLD-FILE)
	python $(SCORER_DIR)/baseline.py --train $(RELEASE_DIR)/$(VERSION)/$*/HIPE-data-$(VERSION)-train-$*.tsv \
	--dev $(RELEASE_DIR)/$(VERSION)/$*/HIPE-data-$(VERSION)-test-$*.tsv \
	--pred $@ --cols NE-COARSE-LIT,NE-COARSE-METO

# take development set to train baseline system for English
$(SUB_DIR)/baseline_bundle4_en_1.tsv: $(EN-GOLD-FILE)
	python $(SCORER_DIR)/baseline.py --train $(RELEASE_DIR)/$(VERSION)/en/HIPE-data-$(VERSION)-dev-en.tsv \
	--dev $(RELEASE_DIR)/$(VERSION)/en/HIPE-data-$(VERSION)-test-en.tsv \
	--pred $@ --cols NE-COARSE-LIT,NE-COARSE-METO
